 ...
 * @file Mat3.h
 * @author Tyler Hanf
 * @brief Provides a Mat3 SIMD implementation
 * @version 0.1
 * @date 2023-08-12
 */
#pragma once
...
public:
  Mat3(void);
  Mat3(const float x0, const float y0, const float z0, const float x1,
...
inline float Mat3::Determinant() const {
  ...
  */
  float detI = M_test[0][0] *
               ((M_test[1][1] * M_test[2][2]) - (M_test[1][2] * M_test[2][1]));
  float detJ = M_test[0][1] *
               ((M_test[1][0] * M_test[2][2]) - (M_test[1][2] * M_test[2][0]));
  float detK = M_test[0][2] *
               ((M_test[1][0] * M_test[2][1]) - (M_test[1][1] * M_test[2][0]));

  return detI - detJ + detK;
}

inline float *Mat3::Ptr(void) { return &(M[0].x); }

inline const float *Mat3::ConstPtr(void) const { return &(M[0].x); }
